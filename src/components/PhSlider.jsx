...
function PhSlider(props){
  const {value, abvGt, abvLt, optionPH, phValue, setBeerData, setOptionPH, setPhValue, optionSRM, srmValue} = props;
  console.log(phValue)
  ...
  function phHandler(phvalue) {
    if (abvGt === true) {
      ...
      fetch(`${data}`)
        ...
        .then((json) => {
          setBeerData(
            json.filter((j) => j.ph != null && phvalue[0] < j.ph && j.ph < phvalue[1])
          );
        });
    }
  }
  ...
  <Col md="auto">
            ...
            />
            <label
              className="form-check-label mx-1 fs-5"
